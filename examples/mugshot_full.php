...
if(!empty($_POST)  && empty($error)) {
	...
	if(empty($post_data['error'])) {
		...
		try{
			...
			try {
				$result2 = $FlashFotoAPI->mugshot($result['Image']['id']);
				try {
					$status = null;
					while(1) {
						sleep(5);
...
<html>
	...
	<body>
		...
		<?php if(isset($result4)): ?>
			...
			<?php else: ?>
				<img src="<?php echo 'data:image/jpeg;base64,'.base64_encode($result4); ?>" alt="MugshotMask"/>
				<?php if(isset($result5)): ?>
					<img src="<?php echo 'data:image/png;base64,'.base64_encode($result5); ?>" alt="MugshotMasked"/>
				<?php endif; ?>
			<?php endif; ?>
		<?php endif; ?>
