...
$method = 'segment status';
$api_url = 'segment_status';
$doc_url = 'segmentstatus';
...
if(!empty($_POST)  && empty($error)) {
	...
	if(empty($post_data['error'])) {
		...
		try{
			$result2 = $FlashFotoAPI->segment_status(!empty($post_data['api_params']['image_id'] ) ? $post_data['api_params']['image_id'] : null);
		} catch(Exception $e) {
			$result = $e;
		}
	} else {
